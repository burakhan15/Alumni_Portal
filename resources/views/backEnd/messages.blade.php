		...
		<div class="col">
			<table class="table table-hover table-bordered results paginated" id="myTable">
				...
				<tbody>
					...
					<tr>
						<td><a href="{{url('admin/look-message/'.$message->id)}}">{{$message->id}}</a></td>
						<td>{{$message->headline}}</td>
						<td>{{$message->created_at}}</td>
						<td>
							<a href="{{url('admin/look-message/'.$message->id)}}" class="btn btn-sm btn-primary"><i class="fa fa-eye"></i> Look Message</a>
							<a href="{{url('admin/del-message/'.$message->id)}}" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i> Delete</a>
						</td>
					</tr>
					@endforeach
				</tbody>
